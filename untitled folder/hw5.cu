...
const int T = 16; // tile size will be t
...
{
  int i, j;
  i = threadIdx.x + (blockDim.x * blockIdx.x); // element (i,j) 
  j = threadIdx.y + (blockDim.y * blockIdx.y); // element (i,j) 
  out[j + (i * N)] = in[i + (j * N)]; // flip i and j to transpose
  return;
}
...
{
    // __shared__ float sharedData[]; // empty matrix in shared memory
    // never mind cannot do an empty matrix
    // Initialize matrix to size N*N. where N*N = 1024, N = 32
    __shared__ float sharedData[1024];


    // same as regular tiled
    int i, j;
    i = threadIdx.x + (blockDim.x * blockIdx.x); // element (i,j) 
    j = threadIdx.y + (blockDim.y * blockIdx.y); // element (i,j) 

    // stops and waits for the other threads to catch up.
    __syncthreads();
